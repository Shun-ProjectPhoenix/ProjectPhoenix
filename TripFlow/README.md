...
5. [Gmail連携：予約メール候補の検索（Web v0.4 Phase A・B）](#gmail連携予約メール候補の検索web-v04-phase-ab)
6. [現在のWeb v0.3でできること](#現在のweb-v03でできること)
   - [ホーム画面](#ホーム画面)
   - [出張管理](#出張管理)
   - [予約管理](#予約管理)
   - [予約詳細カード](#予約詳細カード)
   ...
   - [スマートフォン対応](#スマートフォン対応)
7. [Web v0.2〜v0.3でのコード構成の改善](#web-v02v03でのコード構成の改善)
8. [使用技術](#使用技術)
9. [セットアップ方法](#セットアップ方法)
10. [Googleログインの設定（開発環境）](#googleログインの設定開発環境)
11. [起動方法](#起動方法)
12. [現在のフォルダ構成](#現在のフォルダ構成)
13. [今後のロードマップ](#今後のロードマップ)
14. [今後実装予定の機能（未実装）](#今後実装予定の機能未実装)
15. [現在の制限事項](#現在の制限事項)
16. [開発ステータス](#開発ステータス)
...
## Gmail連携：予約メール候補の検索（Web v0.4 Phase A・B）

Web v0.4では、Gmailに届いた出張関連の予約メールをTripFlow上で検索・確認できる機能の実装を開始しました。**メール本文の解析や、TripFlowへの自動登録はまだ行いません。**

### Phase A：Gmail用アクセストークンの取得

- 既存のGoogleログイン（`st.login()`）のスコープを拡張し、Gmail読み取り専用スコープ（`gmail.readonly`）を追加しました
- ログイン中ユーザー自身のアクセストークンのみを、その場のAPI呼び出しにのみ使用します（DB保存・ログ出力・画面表示は一切行いません）
- アクセストークンが失効した場合は、再ログインを促す案内を表示します（refresh_tokenは取得・保存していません）

### Phase B：Gmail検索・候補一覧表示

- 出張タブ内の「📧 Gmailから予約を探す」から、えきねっと・Agodaの予約関連メールをGmail検索クエリで絞り込んで取得できます
- 検索期間（1／3／6／12ヶ月、デフォルト3ヶ月）、対象サービス（えきねっと／Agoda／両方）を選択できます
- 取得するのは件名・送信元・受信日時・snippet（Gmailが生成する短い抜粋）のみで、メール本文全文は取得・保存しません
- 検索結果はセッション中のみ保持され、ログアウトで消去されます。TripFlowへの登録（DB保存）はまだ行いません
- 実際のGmail環境で、えきねっと・Agodaいずれも検索・候補一覧表示が動作することを確認済みです。なお、予約確認メール以外の案内メール（出発前案内・座席番号案内・レビュー依頼等）も候補に含まれることが分かっており、「予約情報抽出に適したメールかどうかの判定」は次のPhase（Phase C）で扱います
...
- [httpx](https://www.python-httpx.org/)（Streamlitの認証機能が実行時に必要とする依存ライブラリ。Web v0.4からはGmail REST APIの直接呼び出しにも使用）
...
└── web/                          ★ Web v0.4（本体）
    ├── app.py                    Streamlitアプリ本体（ログインゲート・タブ構成・フォーム・カレンダー描画・Gmail検索UI）
    ├── components.py              表示用の定数・整形関数・カード表示コンポーネント
    ├── database.py                SQLiteアクセス層（テーブル定義・CRUD・所有者チェック）
    ├── gmail_service.py           Gmail REST API呼び出し（検索・候補一覧取得。Web v0.4 Phase Bで追加）
    ├── requirements.txt
    └── data/
...
| Phase 3（Web v0.4） | Gmail連携（認証基盤・検索・候補一覧表示） | 🟡 Phase A・B完了、Phase C（本文解析）以降は設計中 |
| Phase 4（Web v0.5〜v0.6） | メール解析精度向上、Googleカレンダー連携、経路案内 | 未着手 |
...
- ~~Gmail API連携~~ → Web v0.4 Phase A・Bで実装済み（検索・候補一覧表示まで）
- Gmailメール本文の解析・予約情報抽出（Phase C予定）
- 予約情報抽出に適したメールかどうかの判定（同一サービス内の案内メール・レビュー依頼メール等との区別。Phase C予定）
- Gmail候補からTripFlowへの登録候補生成・確認画面・登録（Phase C以降予定）
- Gmail取り込みメールの重複登録防止（`gmail_message_id`、Phase D予定）
- Google Calendar連携
- 経路検索（Google Maps連携）
- AIによる予約内容の解析
...
- Gmail連携は検索・候補一覧表示まで実装済みです（Web v0.4 Phase A・B）。メール本文の解析・予約情報抽出・TripFlowへの自動登録はまだ実装していません
...
- **現在：Web v0.4 Phase B（ローカル版）**。Web v0.3までの機能に加え、Gmailの予約メール候補を検索・一覧表示する機能まで実装済みです。メール本文の解析・予約登録候補生成・自動登録はまだ実装していません（Phase C以降で対応予定）
