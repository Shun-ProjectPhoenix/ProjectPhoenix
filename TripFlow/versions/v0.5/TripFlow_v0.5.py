...
def load_trips():
    ...
    for index, trip in enumerate(trips):
        ...
        filtered_count += 1
        total_amount += amount_to_int(trip["金額"])
    ...
    count_label.config(
        text=f"表示中の出張：{filtered_count}件　｜　合計金額：{total_amount:,}円"
    )
...
count_label = tk.Label(root, text="表示中の出張：0件　｜　合計金額：0円", font=("Arial", 11, "bold"))
...
tree.column("trip_name", width=160)
